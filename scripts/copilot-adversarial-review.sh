#!/bin/bash
# GitHub Copilot CLI adversarial code review using copilot -p in non-interactive mode.
# Uses an adversarial prompt stance (assume code will fail) to surface hidden failure paths.
# Normalizes output to the format expected by review-analyze.sh.

REVIEW_NAME="Copilot-Adversarial"
REVIEW_CLI_CHECK="copilot"
REVIEW_INSTALL_HINT="npm install -g @github/copilot OR brew install copilot-cli"
REVIEW_PROMPT_FILE=".claude/copilot-adversarial-code-review-prompt.md"
...
with open(dst, 'w') as f:
    ...
    elif re.search(r'no issues|nothing to report|lgtm|looks good|no material', content, re.IGNORECASE):
        f.write(f'### Code Review Results\n\nNo issues found.\n\n{content}\n')
    else:
        f.write(f'### Code Review Results\n\nFound 0 issues.\n\n{content}\n')
PYEOF
