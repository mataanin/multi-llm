---
name: codex-review
description: Run OpenAI Codex code review using codex exec CLI
---

## Codex Code Review

Run a comprehensive code review using OpenAI's Codex CLI agent with the `codex-review.sh` script.

### Usage

**Review current branch (with change description):**
```bash
./.claude/scripts/codex-review.sh --description "Description of changes"
```

**Review specific commit (with change description):**
```bash
./.claude/scripts/codex-review.sh <commit-hash> --description "Description of changes"
```

**Review commit range (with change description):**
```bash
./.claude/scripts/codex-review.sh <start-commit>..<end-commit> --description "Description of changes"
```

**Alternative syntax for description:**
```bash
./.claude/scripts/codex-review.sh --description="Description of changes" <commit-hash>
```

**Test if codex command is available:**
```bash
./.claude/scripts/codex-review.sh --test
```
...
The script uses the prompt from `.claude/codex-code-review-prompt.md` and runs it through `codex exec` using the **gpt-5-codex** model in read-only sandbox mode.
...
- `codex` command must be available in PATH (install with `npm i -g @openai/codex`)
- Git repository must be initialized
- `.claude/codex-code-review-prompt.md` must exist
- OpenAI API key configured (via `codex` auth or `CODEX_API_KEY` env var)
